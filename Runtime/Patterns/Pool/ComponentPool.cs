...
namespace Bounce.Framework
{
    ...
    public class ComponentPool<T> : ObjectPool<T> where T : Component
    {
        ...
        /// </summary>
        /// <param name="sourcePrefab">The prefab instance you would like to use as a source for T.</param>
        /// <param name="onCreate">Action that will be called when pool entity is created.</param>
        /// <param name="defaultCapacity">The default capacity the stack will be created with.</param>
        public ComponentPool(GameObject sourcePrefab, Action<T> onCreate, int defaultCapacity = 10)
            : base(() =>
            {
                T component = Object.Instantiate(sourcePrefab).GetComponent<T>();
